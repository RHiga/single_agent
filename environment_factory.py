...
class Assembly():
    ...
    def __init__(self, mu=10, sigma=1, product_limit=100):
        self.stock = 0
        self.mu = mu
        self.sigma = sigma
        self.time = 0
    ...
    def work(self):
        ...
        if self.jobp :
            ...
            if (self.time >= self.working_time+1) and (self.product_all < self.product_limit) :
                self.product = 1
                self.product_all += 1
                self.jobp = True
...
class Environment():
    ...
    def reward_func(self, state):
        ...
        # weight of time and reward
        weight = 20 # 10
        end_time = weight*2*(self.column_length-1)

        # run asseymbly workers, input
        self.assembly_in.work()

        # AGV's reward function in Factory

        ## initialize of rewrad at step t
        reward = 0
        ## battery cost
        ...
        # reward function in product line
        reward += self.assembly.product \
        -(self.assembly.stock + self.assembly_in.product_all)/end_time

        # total time in product line
        self.total_time += 1
        if self.total_time == end_time:
            done = True
        return reward, done
    ...
    def render(self, state, done):
        if not done:
            self.path.append([state.row, state.column])
        else:
            fig, ax = plt.subplots(figsize=(5, 5))
            locs = list(range(len(self.path)))
            def plot(i):
                ax.clear()
                # plot map
                sns.heatmap(self.grid,square=True,cbar=False,cmap='gray_r',
                linewidths=1, linecolor='Black',
                fmt='s',xticklabels=False, yticklabels=False)
                # plot agent
                plt.plot(self.path[i][1]+0.5,self.path[i][0]+0.5, 'o',
                markersize=20, color='r',alpha=0.7)

            agent_animation = FuncAnimation(fig, plot, frames=len(locs), repeat=False)
            #return HTML(anim.to_jshtml())
            return agent_animation
